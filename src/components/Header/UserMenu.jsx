...
import { Avatar, Button, IconButton, Menu, MenuItem, Typography, withStyles } from '@material-ui/core'
        ...
        <div className={classes.desktop}>
          {auth.user ?
            <div className={classes.flex} onClick={this.handleMenu}>
              <Avatar>{initialsFromUsername(auth.user.name)}</Avatar>
              <Typography className={classes.userName} variant="subheading" color="secondary">
                {auth.user.name}
              </Typography>
            </div>
            :
            <React.Fragment>
              <Link to="/auth/login"><Button color="secondary">Войти</Button></Link>
              <Link to="/auth/register"><Button color="secondary">Зарегистрироваться</Button></Link>
            </React.Fragment>
          }
        </div>
